...
import { AgentHeroSection } from "@/components/team/AgentHeroSection";
...
export async function TeamMemberProfile({ member }: { member: TeamMember }) {
  ...
  // Pick locale-appropriate video URL
  const resolvedVideoUrl =
    locale === "en" && member.videoUrlEn
      ? member.videoUrlEn
      : member.videoUrl;

  const firstName = member.name.split(" ")[0];

  const languageLabels: Partial<Record<"en" | "es" | "pt", string>> = {
    en: tc("english"),
    es: tc("spanish"),
    pt: tc("portuguese"),
  };
  ...
  return (
    <main className="flex flex-1 flex-col">
      ...
      {resolvedVideoUrl ? (
        /* ── SECTION 1 (video): current dark hero layout ── */
        <section className="bg-navy-950 pb-14 pt-8">
          <div className="mx-auto max-w-5xl px-6">
            ...
            <div className="relative mt-5 pl-6">
              <span aria-hidden="true" className="absolute left-0 top-0 font-display text-3xl leading-none text-gold">
                &ldquo;
              </span>
            ...
            <ul className="mt-5 flex flex-wrap gap-2">
              {member.languages.map((lang) => (
                <li key={lang} className="rounded-full border border-navy-600 bg-navy-800 px-3.5 py-1 text-sm font-medium text-cream">
            ...
            <div className="mt-8">
              <AgentVideoSection
                ...
                name={member.name}
                label={t("profile.videoLabel", { name: firstName })}
                large
              />
            </div>
            ...
            <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
              ...
              >
                {t("profile.contactCta", { name: firstName })}
        ...
        /* ── SECTION 1 (no video): full-bleed agent hero ── */
        <AgentHeroSection
          member={member}
          backLabel={t("profile.back")}
          tagline={tagline}
          languageLabels={languageLabels}
          contactCta={t("profile.contactCta", { name: firstName })}
          callCta={t("profile.callCta")}
        />
      )}
      ...
      <section className="bg-cream py-12 lg:py-16">
        <div className="mx-auto max-w-5xl px-6">
          <div className="grid gap-10 lg:grid-cols-[1fr_300px] lg:gap-14">
            ...
            <FadeInSection direction="right" delay={0.1} className="hidden lg:block">
              <div className="relative aspect-4/5 overflow-hidden rounded-sm bg-linear-to-br from-navy-800 to-slate">
                {member.bioPhotoUrl ? (
                  <Image src={member.bioPhotoUrl} alt={member.name} fill sizes="300px" className="object-cover object-top" />
                ) : (
      ...
      {resource && (
        <section className="border-t border-navy-100 bg-white py-12 lg:py-16">
          <div className="mx-auto max-w-5xl px-6">
                ...
                return (
                  <li key={m.slug}>
                    <FadeInSection delay={idx * 0.07}>
                      ...
                      >
                        <div className={`relative h-11 w-11 shrink-0 overflow-hidden rounded-full ${avatarAccent.bg}`}>
                          {m.photoUrl ? (
                            <Image src={m.photoUrl} alt={m.name} fill sizes="44px" className="object-cover" />
                          ) : (
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="block truncate font-display text-base font-medium leading-snug text-cream">
                            {m.name}
                          </span>
